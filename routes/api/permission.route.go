...
func SetupPermissionRoutes(api fiber.Router, auth *middlewares.AuthMiddleware) {
	h := api_handlers.NewPermissionHandler(repositories.NewPermissionRepo(database.GetDB()))

	g := api.Group("/permission")
